...
External::check(const std::vector<ObjectIdx>& args )  {
    ...
    //cout << "This is action numbers: " << a1 <<a2<<a3<<a4<<a5<<a6 << endl;
    int action_sequences[ACTION_NUMBER]={a1,a2,a3,a4,a5,a6};
        ...
        for (auto query_detail : query_detail_list)
        {
            for (int depth = 0; depth < (query_detail.size() - 1) / 2;depth++)
            {
                ...
                for (auto object: *state1->getObjects()) {
                    ...
                    string temp_obj_name = "";
                    //cout << "Object name is " << obj_name << endl;
                    //cout << "Object name size " << obj_name.size() << endl;
                    //cout << "(depth+2) * 2 is " << (depth+2) * 2 << endl;

                    if (obj_name.size() < (depth) * 2) {
                        continue;
                    }
                    else
                    {
                        ...
                        if (obj_id != obj_name.back()) {
                            ...
                            temp_obj_name.insert(temp_obj_name.begin(), obj_name.back());
                            temp_obj_name.insert(temp_obj_name.begin(), 's');
                            obj_name.pop_back();
                            obj_name.pop_back();
                            //cout << "String is " << temp_query_detail.back() << endl;
                            //cout << "Current Obj name is " << obj_name << endl;
                            temp_query_detail.pop_back();
                            temp_depth--;
                            //cout << "temp depth is " << temp_depth << endl;
                            while (temp_depth >=0) {

                                if ((obj_name.back() != temp_query_detail.back()[0])||(obj_name[1] !=a1&&obj_name[1] !=a2)) {
                                    relative = false;
                                    //cout << "False here" << endl;
                                    break;
                                }
                        ...
                        if (relative) {
                            ...
                            if (!state1->findObject(name1)) {
                                ...
                                temp.addVariable("value", "1");
                                temp_objects.push_back(temp);
                                //cout << "adding temp sting: " << name1 << endl;
                            }
                            string name2 = "k";
                            name2.push_back(a2);
                            name2.append(temp_obj_name);
                            if (!state1->findObject(name2)&&name1!=name2) {
                                ...
                                temp.addVariable("value", "1");
                                temp_objects.push_back(temp);
                                //cout << "adding temp sting: " << name2 << endl;
                            }
                        }
